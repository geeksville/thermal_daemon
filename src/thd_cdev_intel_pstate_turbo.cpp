...
void cthd_cdev_intel_pstate_turbo::set_curr_state(int state, int arg) {
	...
	if (cdev_sysfs.exists(tc_state_dev.str())) {
		...
		if (cdev_sysfs.write(tc_state_dev.str(), state_str.str()) < 0)
			...
			curr_state = state;
		if (!new_state) {
			// Quirk which willl elevate min_perf_pct or
			// decrease max_perf_pct and
			// will bring back to the current val
			// This will force to enagage turbo in all cores ASAP
			std::stringstream tc_pc_state_dev;
			tc_pc_state_dev << "/max_perf_pct";
			std::string min_perf_str;
			int ret = cdev_sysfs.read(tc_pc_state_dev.str(), min_perf_str);
			if (ret >= 0) {
				ret = cdev_sysfs.write(tc_pc_state_dev.str(), "50");
				if (ret < 0) {
					thd_log_warn(
							"cthd_cdev_intel_pstate_turbo:set_curr_state failed\n");
				}
				sleep(1);
				cdev_sysfs.write(tc_pc_state_dev.str(), min_perf_str);
			}
		}
	} else
		curr_state = (state == 0) ? 0 : max_state;
